// nnet3bin/nnet3-train-discriminative.cc
...
int main(int argc, char *argv[]) {
  try {
    ...
    const char *usage =
        ...
        "\n"
        "Usage:  nnet3-train-discriminative [options] <model> <lat> <ali> <training-examples-in> <raw-model-out>\n"
        "\n"
        "e.g.:\n"
        "nnet3-train-discriminative 0.mdl 'ark:nnet3-merge-egs 1.egs ark:-|' 2.raw\n";
    ...
    std::string mdl_rxfilename = po.GetArg(1),
        clat_rspecifier = po.GetArg(2),
        ref_ali_rspecifier = po.GetArg(3),
    ...
    bool ok;
    {
      ...
      for (; !example_reader.Done() && !clat_reader.Done(); example_reader.Next(), clat_reader.Next()) {
          ...
          trainer.Train(example_reader.Value(), clat, ref_ali);
          stats.Print(train_config.criterion);

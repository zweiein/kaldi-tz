...
common_egs_dir=exp/nnet3/lstm_joint_i/egs
dir=exp/nnet3/lstm_joint_g_no_ld

if [ $stage -le 8 ]; then
  steps/nnet3/lstm/train_speech_spk.sh --stage $train_stage \
...
if [ $stage -le 9 ]; then
  ...
  for lm_suffix in bd_tgpr; do
    ...
    for year in dev93 eval92 eval93 all; do
	 ...
	 $graph_dir data_fbank/test_${year} $dir/decode_${lm_suffix}_${year} || exit 1;
      ) &
    done
  done
fi

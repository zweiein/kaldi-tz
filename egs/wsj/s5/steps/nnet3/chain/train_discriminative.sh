...
num_jobs_nnet=8    # Number of neural net jobs to run in parallel.  Note: this
...
num_threads=16  # this is the default but you may want to change it, e.g. to 1 if
                # using GPUs.
#parallel_opts="--num-threads 16"  #parallel_opts="--num-threads 16 --mem 1G" #by default we use 4 threads; this lets the queue know.
...
#    nnet3-copy-egs $const_dim_opt ark:- $egs_list || exit 1;
  ...
  egs_opts="--left-context=$left_context --right-context=$right_context --frame-subsampling-factor=$frame_subsampling_factor --compress=true"
  $cmd JOB=1:$nj $dir/log/get_egs.JOB.log \
    ...
    chain-get-supervision --lattice-input=true --frame-subsampling-factor=$frame_subsampling_factor $chain_tree $ctc_trans_mdl ark:- ark:- \| \
    nnet3-chain-get-egs-per-utt $egs_opts "$feats" ark,s,cs:- ark:- \| \
    nnet3-chain-copy-egs ark:- $egs_list || exit 1;
...
if [ $stage -le -5 ] && [ -z "$degs_dir" ]; then
  ...
  if [ $iters_per_epoch -eq 1 ]; then
    ...
    $cmd $io_opts JOB=1:$num_jobs_nnet $dir/log/split_egs.JOB.log \
      nnet3-chain-copy-egs --srand=JOB \
...
if [ $stage -le -4 ] && [ -z "$degs_dir" ]; then
  ...
  nnet3-chain-shuffle-egs --srand=JOB "ark:cat $dir/degs/degs_tmp.JOB.*.ark|" ark:- \| \
  nnet3-chain-copy-egs ark:- ark:$dir/degs/degs.JOB.ark || exit 1; 
  ...
  for n in `seq 1 $num_jobs_nnet`; do
      nnet3-chain-copy-egs ark:$degs_dir/degs.$n.ark ark,scp:$dir/$n.ark,$dir/$n.scp;
...
while [ $x -lt $num_iters ]; do
  if [ $stage -le $x ]; then
    ...
    $cmd JOB=1:$num_jobs_nnet $dir/log/train.$x.JOB.log \
      nnet3-train-discriminative$parallel_suffix  --silence-phones=$silphonelist \
       --criterion=$criterion --drop-frames=$drop_frames \
       --one-silence-class=$one_silence_class --boost=$boost \
       --acoustic-scale=$acoustic_scale \
        $dir/$x.mdl "scp:$dir/lat.JOB.scp" "$ali" \
        "ark:nnet3-chain-copy-egs scp:$dir/JOB.scp ark:- |" \

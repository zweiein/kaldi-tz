...
add_joint1=10 # when to add undirectional iteration between speech and spk
add_joint2=20 # when to add the other
...
if [ $# != 5 ]; then
  echo "Usage: $0 [opts] <data> <lang> <ali-dir> <spk-ali-dir> <exp-dir>"
...
for f in $dir/configs/all.config $dir/configs/all_joint1.config $dir/configs/all_joint2.config $dir/configs/vars; do
...
while [ $x -lt $num_iters ]; do
  ...
  if [ $x -ge 0 ] && [ $stage -le $x ]; then
    ...
    # add interactions between speech and spk
    add_joint1=$[$num_iters/3]
    add_joint2=$[($num_iters/3)*2]
    if [ $x -eq $add_joint1 ]; then
      raw="nnet3-am-copy --raw=true --learning-rate=$this_learning_rate $dir/$x.mdl -| nnet3-init --srand=$x - $dir/configs/all_joint1.config -|"
    fi
    if [ $x -eq $add_joint2 ]; then
      raw="nnet3-am-copy --raw=true --learning-rate=$this_learning_rate $dir/$x.mdl -| nnet3-init --srand=$x - $dir/configs/all_joint2.config -|"
    fi

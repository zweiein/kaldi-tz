...
add_joint=10 # when to add iteration between speech and spk
...
for f in $dir/configs/all.config $dir/configs/all_joint.config $dir/configs/vars; do
...
while [ $x -lt $num_iters ]; do
  ...
  if [ $x -ge 0 ] && [ $stage -le $x ]; then
    ...
    if [ $x -eq $add_joint ]; then
      raw="nnet3-am-copy --raw=true --learning-rate=$this_learning_rate $dir/$x.mdl -| nnet3-init --srand=$x - $dir/configs/all_joint.config -|"
    fi

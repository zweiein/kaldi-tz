  ...
  if [ $x -ge 0 ] && [ $stage -le $x ]; then
    ...
    ( # this sub-shell is so that when we "wait" below,
      ...
      for n in $(seq $this_num_jobs); do
        ...
        if [ $[$x%$self_train_period] -eq 0 ] && [ $x -ge $start_to_self_train ]; then

          nnet3-copy --binary=false "$raw" $dir/self.raw
          sed -i "s/input=final-log-softmax/input=final-fixed-scale/" $dir/self.raw
          raw=$dir/self.raw
          ...
          soft_targets="ark:nnet3-compute-from-egs --use-gpu=yes \"$raw\" \"$final_egs\" ark:- |"

          $cmd $train_queue_opt $dir/log/self_train.$x.$n.log \

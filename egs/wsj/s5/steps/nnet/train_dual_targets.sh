...
if [ ! -z "$labels" ]; then
  ...
  labels_hard_tr="ark:ali-to-pdf $alidir/final.mdl \"ark:gunzip -c $alidir/ali.*.gz |\" ark:- | ali-to-post ark:- ark:- |"
  labels_hard_cv="ark:ali-to-pdf $alidir/final.mdl \"ark:gunzip -c $alidir_cv/ali.*.gz |\" ark:- | ali-to-post ark:- ark:- |"
  labels_soft_tr="ark:nnet-forward --use-gpu=yes --temperature=$temperature --feature-transform=$ref_dir/final.feature_transform $ref_dir/final.nnet \"$feats_tr\" ark:- |"
  labels_soft_cv="ark:nnet-forward --use-gpu=yes --temperature=$temperature --feature-transform=$ref_dir/final.feature_transform $ref_dir/final.nnet \"$feats_cv\" ark:- |"
  # 
...
steps/nnet/train_scheduler_dual_targets.sh \
  ...
  $mlp_init "$feats_tr" "$feats_cv" "$labels_hard_tr" "$labels_hard_cv" "$labels_soft_tr" "$labels_soft_cv" $dir || exit 1

...
if [ $# != 4 ]; then
   echo "usage: steps/align_nofmllr_lats.sh <data-dir> <lang-dir> <src-dir> <align-dir>"
   echo "e.g.:  steps/align_nofmllr_lats.sh data/train data/lang exp/tri1 exp/tri1_lats"
...
if [ $stage -le 1 ]; then
  ...
  $cmd JOB=1:$nj $dir/log/align_pass1.JOB.log \
    ...
    "ark:gunzip -c $dir/fsts.JOB.gz|" "$feats" "ark:|gzip -c >$dir/pre_ali.JOB.gz" || exit 1;
fi
